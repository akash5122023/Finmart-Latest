...
Tenant subdomain creation now uses Cloudflare's DNS API. The sample `appsettings.json` includes `Cloudflare:ZoneId` `dd9fd7de1f7586dad10ceea8aada6076` and `Cloudflare:ServerIp` `103.61.224.79` for `bizpluserp.com`; supply `Cloudflare:ApiToken` (or set the `CLOUDFLARE_API_TOKEN` environment variable) so the application can create proxied A records like `tenant.bizpluserp.com` for new tenants. Other values (`CLOUDFLARE_ZONE_ID`, `CLOUDFLARE_SERVER_IP`, `CLOUDFLARE_ROOT_DOMAIN`) can also be overridden via environment variables if needed.
