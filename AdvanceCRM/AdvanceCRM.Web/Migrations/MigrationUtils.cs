...
namespace AdvanceCRM.Migrations
{
...
END;", table, id, seq));
        ...
        public static void CreateTableIfNotExists(
            this MigrationBase migration, string table,
            Action<ICreateTableWithColumnSyntax> addColumns,
            string schema = null)
        {
            if (schema != null)
            {
                if (migration.Schema.Schema(schema).Table(table).Exists())
                    return;

                addColumns(migration.Create.Table(table).InSchema(schema));
            }
            else
            {
                if (migration.Schema.Table(table).Exists())
                    return;

                addColumns(migration.Create.Table(table));
            }
        }
    }
