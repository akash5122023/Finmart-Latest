...
namespace AdvanceCRM
{
    public class DataMigrations : IDataMigrations
    {
        ...
        private void RunMigrations(string databaseKey)
        {
            ...
            // NOTE: Original template had a safety check here that only ran
            // migrations if the connection string contained a specific marker
            // (e.g. "AdvanceCRM_Default_v1"). This blocked migrations when
            // pointing to a new database name like a fresh customer DB.
            //
            // For this deployment we always want migrations to run against the
            // configured connection, so the safety check is disabled.
            //
            // If you ever need to re‑enable it, restore the check below:
            //
            ...
            {
                // No migrations defined for this database key (e.g. Log DB).
                // Treat as a no-op instead of failing application startup.
                SkippedMigrations = true;
            }
            catch (Exception ex)
            {
