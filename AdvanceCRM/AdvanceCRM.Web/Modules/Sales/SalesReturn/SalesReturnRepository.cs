...
namespace AdvanceCRM.Sales.Repositories
{
    ...
    public class SalesReturnRepository : BaseRepository
    {
        protected ISqlConnections _connections { get; }

        public SalesReturnRepository(IRequestContext context, ISqlConnections connections)
            : base(context)
        {
            _connections = connections;
        }

        public SalesReturnRepository(IRequestContext context) : base(context) { }
        ...
        {
            private readonly SalesReturnRepository _repository;

            public MySaveHandler(SalesReturnRepository repository) : base(repository.Context)
            {
            ...
            protected override void AfterSave()
            {
                ...
                notify.Module = Masters.NotificationModules.SalesReturn;
                notify.InsertDate = System.DateTime.Now;
                notify.InsertUserId = Convert.ToInt32(Context.User.GetIdentifier());
                notify.Url = "/SalesReturn#edit/" + Request.Entity.Id;

                notify.Text = Serenity.Authorization.Username + " has" + (this.IsUpdate ? " Updated" : " Created") + " a Sales Return";
        ...
        private class MyListHandler : ListRequestHandler<MyRow>
        {
            ...
            public MyListHandler(SalesReturnRepository repository) : base(repository.Context)
            {
                _repository = repository;
            }
