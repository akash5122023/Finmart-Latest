...
namespace AdvanceCRM.Sales
{
    ...
    [DisplayName("Proforma"), InstanceName("Proforma")]
    [ReadPermission("Proforma:Read")]
    [InsertPermission("Proforma:Insert")]
    [UpdatePermission("Proforma:Update")]
    [DeletePermission("Proforma:Delete")]
    [LookupScript("Sales.Proforma", Permission = "?")]
    public sealed class InvoiceRow : Row<InvoiceRow.RowFields>, IIdRow, INameRow, _Ext.IAuditLog, IMultiCompanyRow
    {
        ...
        [MultipleImageUploadEditor(FilenameFormat = "Invoice/~", CopyToHistory = true, AllowNonImage = true)]
        ...
        [DisplayName("Project"), Expression("jContactPerson.[Project]"), LookupInclude]
        ...
        [DisplayName("Total"), Expression("(SELECT SUM((Price * Quantity) - ((DiscountAmount) + ((Price * Quantity) * (Discount / 100)))) FROM InvoiceProducts WHERE InvoiceId=t0.[Id])"), AlignRight]
        public Double? Total { get { return Fields.Total[this]; } set { Fields.Total[this] = value; } }

        [DisplayName("Tax1"), Expression("(SELECT SUM(((Price * Quantity) - (DiscountAmount + (Price * Quantity * (Discount / 100)))) * (Percentage1 / 100)) FROM InvoiceProducts WHERE InvoiceId=t0.[Id])"), AlignRight]
        public Double? Tax1 { get { return Fields.Tax1[this]; } set { Fields.Tax1[this] = value; } }

        [DisplayName("Tax2"), Expression("(SELECT SUM(((Price * Quantity) - (DiscountAmount + (Price * Quantity * (Discount / 100)))) * (Percentage2 / 100)) FROM InvoiceProducts WHERE InvoiceId=t0.[Id])"), AlignRight]
        public Double? Tax2 { get { return Fields.Tax2[this]; } set { Fields.Tax2[this] = value; } }

        [DisplayName("Amount"), Expression("IIF([Taxable] = 0, (SELECT SUM((Price * Quantity) - ((DiscountAmount) + ((Price * Quantity) * (Discount / 100)))) FROM InvoiceProducts WHERE InvoiceId=t0.[Id]), (SELECT SUM((((Price * Quantity) - ((DiscountAmount) + ((Price * Quantity) * (Discount / 100)))) + (((Price * Quantity) - ((DiscountAmount) + ((Price * Quantity) * (Discount / 100)))) * (Percentage1 / 100)) + (((Price * Quantity) - ((DiscountAmount) + ((Price * Quantity) * (Discount / 100)))) * (Percentage2 / 100)))) FROM InvoiceProducts WHERE InvoiceId=t0.[Id])) + (t0.[PackagingCharges] + t0.[FreightCharges] + t0.[Roundup] - t0.[Advacne])"), AlignRight]
        public Double? GrandTotal { get { return Fields.GrandTotal[this]; } set { Fields.GrandTotal[this] = value; } }

        [DisplayName("Products"), MasterDetailRelation(foreignKey: "InvoiceId", IncludeColumns = "ProductsName"), NotMapped]
        public List<InvoiceProductsRow> Products { get { return Fields.Products[this]; } set { Fields.Products[this] = value; } }

        [DisplayName("Terms")]
        [LookupEditor(typeof(QuotationTermsMasterRow), Multiple = true, InplaceAdd = true), NotMapped]
        [LinkingSetRelation(typeof(InvoiceTermsRow), "InvoiceId", "TermsId")]
        ...
        [LinkingSetRelation(typeof(InvoiceChargesRow), "InvoiceId", "ChargesId")]
        ...
        [LinkingSetRelation(typeof(InvoiceConcessionRow), "InvoiceId", "ConcessionId")]
        ...
        public class RowFields : RowFieldsBase
        {
            ...
            public readonly RowListField<InvoiceProductsRow> Products;
