...
namespace AdvanceCRM.Sales.Endpoints
{
    ...
    using MyRepository = Repositories.InvoiceAppointmentsRepository;
    using MyRow = InvoiceAppointmentsRow;

    [Route("Services/Sales/InvoiceAppointments/[action]")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class InvoiceAppointmentsController : ServiceEndpoint
    {
        private readonly ISqlConnections _connections;
        private readonly IWebHostEnvironment _env;

        public InvoiceAppointmentsController(ISqlConnections connections, IWebHostEnvironment env)
        {
        ...
        [ServiceAuthorize("Reports:Proforma:Appointments")]
        public FileContentResult ListExcel(IDbConnection connection, ListRequest request)
        {
            var data = List(connection, request).Entities;
            var report = new DynamicDataReport(data, request.IncludeColumns, typeof(Columns.InvoiceAppointmentsColumns));
            var bytes = new ReportRepository().Render(report);
            return ExcelContentResult.Create(bytes, "InvoiceAppointments_" +
