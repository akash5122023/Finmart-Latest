...
namespace AdvanceCRM.Sales {
    ...
    export class SalesProductsEditorDialog extends Common.GridEditorDialog<SalesProductsRow> {
        protected getFormKey() { return SalesProductsForm.formKey; }
        protected getLocalTextPrefix() { return SalesProductsRow.localTextPrefix; }
        protected getNameProperty() { return SalesProductsRow.nameProperty; }
        protected form = new SalesProductsForm(this.idPrefix);
        ...
        constructor() {
            ...
            this.form.ProductsId.changeSelect2(e => {
                ...
                if (PID != null) {
                    ...
                    //Q.notifyInfo("Min stock: " + minstock.toString());
                    //if ((minstock != 0) || (minstock != null)) {
                    //    Products.StockTransferService.CheckReorder({ Id: PID },
                    //        response => {
                              
                    //            if (Q.parseInteger(response.Status) <= minstock) {
                    ...
                    //            }
                    //            else { Q.notifyInfo("Total stock: " + response.Status.toString());}
                    //        });
                    //}
                    var minstock = Products.ProductsRow.getLookup().itemById[PID].MinimumStock;
                    Products.StockTransferService.CheckReorder({ Id: PID },
            ...
            this.form.Serial.change(e => {

                var serial = Q.text(this.form.Serial.value);

                //Q.notifyInfo(serial);

                this.form.ProductsId.value = Q.tryFirst(SalesProductsRow.getLookup().items, x => x.Serial == serial).ProductsId.toString();

                var sold = Q.toId(Q.tryFirst(SalesProductsRow.getLookup().items, x => x.Serial == serial).Id);

                Q.notifyInfo(sold);

                if (sold != null) {
                    Q.notifyError("This item is already sold");
                    this.form.Serial.value = "";
                }
                else {
                    var PID = Q.toId(Q.tryFirst(SalesProductsRow.getLookup().items, x => x.Serial == serial).Id);

                    this.form.Mrp.value = SalesProductsRow.getLookup().itemById[PID].Mrp;
                    this.form.SellingPrice.value = SalesProductsRow.getLookup().itemById[PID].SellingPrice;
                    this.form.Price.value = SalesProductsRow.getLookup().itemById[PID].Price;
                    this.form.TaxType1.value = SalesProductsRow.getLookup().itemById[PID].TaxType1;
                    this.form.Percentage1.value = SalesProductsRow.getLookup().itemById[PID].Percentage1;
                    this.form.TaxType2.value = SalesProductsRow.getLookup().itemById[PID].TaxType2;
                    this.form.Percentage2.value = SalesProductsRow.getLookup().itemById[PID].Percentage2;
                    this.form.WarrantyStart.value = SalesProductsRow.getLookup().itemById[PID].WarrantyStart;
                    this.form.WarrantyEnd.value = SalesProductsRow.getLookup().itemById[PID].WarrantyEnd;
                }
            });
        }
        ...
        onDialogOpen() {
            ...
            if (Administration.CompanyDetailsRow.getLookup().itemById[1].CodeInSales == true) {
                this.form.Code.getGridField().toggle(false);
            }

            if (Administration.CompanyDetailsRow.getLookup().itemById[1].SerialInSales == true) {
                this.form.Serial.getGridField().toggle(false);
            }

            if (Administration.CompanyDetailsRow.getLookup().itemById[1].BatchInSales == true) {
                this.form.Batch.getGridField().toggle(false);
            }

            if (Administration.CompanyDetailsRow.getLookup().itemById[1].DiscountInSales == true) {
                this.element.find(".category-title:contains('Discount')").parent().toggle(false);
                this.form.Discount.getGridField().toggle(false);
                this.form.DiscountAmount.getGridField().toggle(false);
            }

            if (Administration.CompanyDetailsRow.getLookup().itemById[1].TAXInSales == true) {
                this.element.find(".category-title:contains('TAX')").parent().toggle(false);
                ...
                this.form.Inclusive.getGridField().toggle(false);
            }

            if (Administration.CompanyDetailsRow.getLookup().itemById[1].WarrantyInSales == true) {
                this.element.find(".category-title:contains('Warranty')").parent().toggle(false);
                this.form.WarrantyStart.getGridField().toggle(false);
                this.form.WarrantyEnd.getGridField().toggle(false);
            }

            if (Administration.CompanyDetailsRow.getLookup().itemById[1].DescriptionInSales == true) {
                this.form.Description.getGridField().toggle(false);
            }
