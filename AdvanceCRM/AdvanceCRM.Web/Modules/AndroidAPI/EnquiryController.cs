...
namespace AdvanceCRM.Modules.AndroidAPI
{
    ...
    public class EnquiryController : ControllerBase
    {
        ...
        public EnquiryController(IConfiguration config)
        {
        ...
        [HttpGet]
        public IActionResult Get([FromQuery] Paginator filter)
        {
            var paginator = new Paginator(filter.per_page, filter.current_page);

            using (SqlConnection con = new SqlConnection(_connectionString))
            {
                string str = "SELECT en.Id, cn.Name, cn.Phone, en.Date, Status, sr.Source, st.Stage, us.DisplayName, en.AssignedId, en.EnquiryN, en.EnquiryNo FROM Enquiry en, Source sr, Stage st, Contacts cn, Users us WHERE us.UserId = en.OwnerId AND en.ContactsId = cn.Id AND st.Id = en.stageId AND sr.Id = en.SourceId";
                ...
                var result = Contact.Skip((paginator.current_page - 1) * paginator.per_page)
                                   .Take(paginator.per_page)
                                   .ToList();
        ...
        public IActionResult Get(string id, [FromQuery] EnquiryNModel pagingparametermodel)
        {
            using (SqlConnection con = new SqlConnection(_connectionString))
            {
                ...
                string str = "SELECT en.Id, cn.Name, cn.Phone, cn.Address, en.Date, Status, sr.Source, st.Stage, en.EnquiryN, en.EnquiryNo, us.DisplayName, en.AssignedId FROM Enquiry en, Source sr, Stage st, Contacts cn, Users us WHERE us.UserId = en.OwnerId AND en.ContactsId = cn.Id AND st.Id = en.stageId AND sr.Id = en.SourceId AND en.AssignedId = @AssignedId ORDER BY en.Id DESC";
