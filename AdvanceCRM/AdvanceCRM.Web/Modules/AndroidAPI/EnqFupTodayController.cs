...
namespace AdvanceCRM.Modules.AndroidAPI
{
    ...
    public class EnqFupTodayController : ControllerBase
    {
        List<EnquiryFollowupModel> Contact;
        private readonly string _connectionString;

        public EnqFupTodayController(IConfiguration config)
        {
        ...
        public IActionResult Get(string id, [FromQuery] EnquiryNModel pagingparametermodel)
        {
            using (SqlConnection con = new SqlConnection(_connectionString))
            {
                ...
                string str = "SELECT cn.Name, cn.Phone, enq.EnquiryN, enq.EnquiryNo, enq.StageID, enq.SourceID, sr.Source, st.Stage, en.Id, en.FollowUpDate, en.Status, en.FollowUpNote, en.EnquiryId, en.Details, us.DisplayName FROM EnquiryFollowUps en, Enquiry enq, Source sr, Stage st, Users us, Contacts cn WHERE en.RepresentativeId = us.UserId AND enq.ContactsId = cn.Id AND en.EnquiryId = enq.Id AND st.Id = enq.stageId AND sr.Id = enq.SourceId AND en.FollowupDate >= @StartDate AND en.FollowupDate <= @EndDate AND en.RepresentativeId = @RepresentativeId ORDER BY en.Id DESC";
