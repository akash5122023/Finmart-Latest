...
namespace AdvanceCRM.Template.Repositories
{
    ...
    using System;
    using System.Data;
    using MyRow = CmsTemplateRow;
    using Serenity.Extensions.DependencyInjection;

    public class CmsTemplateRepository : BaseRepository
    {
        private readonly ISqlConnections _sqlConnections;

        public CmsTemplateRepository(IRequestContext context, ISqlConnections connections) : base(context)
        {
            _sqlConnections = connections;
        }

        public CmsTemplateRepository(IRequestContext context)
        ...
        public RetrieveResponse<MyRow> Retrieve(IDbConnection connection, RetrieveRequest request)
        {
            if ((long)request.EntityId == -1)
            {
                ...
                using (var conn = _sqlConnections.NewFor<MyRow>())
                {
                    var f = MyRow.Fields;
