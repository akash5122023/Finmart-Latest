...
namespace AdvanceCRM.FinmartInsideSales {
    ...
    export class InsideSalesGrid extends GridBase<InsideSalesRow, any> {
        protected getColumnsKey() { return 'FinmartInsideSales.InsideSales' }
        protected getDialogType() { return InsideSalesDialog; }
		...
		protected getButtons(): Serenity.ToolButton[] {
			...
			buttons.push({
				...
				icon: 'fa-file-excel',
				onClick: () => {
					const url = '/Services/FinmartInsideSales/InsideSales/ListExcel';
				...
				onClick: () => {
					...
					fileInput.onchange = () => {
						if (fileInput.files && fileInput.files.length > 0) {
							...
							fetch('/Services/FinmartInsideSales/InsideSales/ImportExcel', {
								method: 'POST',
								body: formData
