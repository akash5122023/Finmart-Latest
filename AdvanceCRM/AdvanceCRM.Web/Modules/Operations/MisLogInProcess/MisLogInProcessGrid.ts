...
namespace AdvanceCRM.Operations {
    ...
    export class MisLogInProcessGrid extends GridBase<MisLogInProcessRow, any> {
        protected getColumnsKey() { return 'Operations.MisLogInProcess' }
        protected getDialogType() { return MisLogInProcessDialog; }
		...
		protected getButtons(): Serenity.ToolButton[] {
			...
			buttons.push({
				...
				icon: 'fa-file-excel',
				onClick: () => {
					const url = '/Services/Operations/MisLogInProcess/ListExcel';
				...
				onClick: () => {
					...
					fileInput.onchange = () => {
						if (fileInput.files && fileInput.files.length > 0) {
							...
							fetch('/Services/Operations/MisLogInProcess/ImportExcel', {
								method: 'POST',
								body: formData
