...
namespace AdvanceCRM.Common.LargeFileUpload
{
    public class FileUploadController : ApiController
    {
        ...
        public FileUploadController(IConfiguration configuration, IWebHostEnvironment env, ITextLocalizer localizer)
        {
        ...
        public async Task<IHttpActionResult> TemporaryLargeFileUpload()
        {
            ...
            try
            {
                var uploadFileService = new UploadFileService(_configuration, _env);
                ...
                if (uploadResult.IsComplete)
                {
                    ...
                    if (IsImageExtension(saveFileName))
                    {
                        ...
                        using (var fileContent = new FileStream(saveFilePath, FileMode.Open))
                        {
                            ...
                            {
                                throw new Exception(imageChecker.FormatErrorMessage(checkResult, _localizer));
                            }
                            else
                            {
                                using (var thumbImage = ThumbnailGenerator.Generate(image, 128, 96, ImageScaleMode.CropSourceImage, Color.Transparent))
                                {
                                    var thumbFile = Path.Combine(Path.GetDirectoryName(uploadResult.LocalFilePath), baseFileName + "_t.jpg");
                                    thumbImage.SaveAsJpeg(thumbFile);
