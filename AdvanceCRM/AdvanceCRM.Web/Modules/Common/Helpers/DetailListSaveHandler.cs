...
{
    public class DetailListSaveHandler<TRow, TFields>
        where TRow : Row<TFields>, IIdRow, new()
        where TFields : RowFieldsBase, new()
    {
        ...
        public DetailListSaveHandler(IRequestContext context, IEnumerable<TRow> oldList, IEnumerable<TRow> newList, Action<TRow> setOwnerID,
        ...
        {
            new DeleteRequestHandler<TRow>(_context).Process(uow, request);
        }

        protected virtual void Save(IUnitOfWork uow, SaveRequest<TRow> request)
        {
            new SaveRequestHandler<TRow>(_context).Process(uow, request);
        }

        private long? GetID(TRow row) =>
            ((Int64Field)row.IdField)[row];
        ...
        public virtual void Process(IUnitOfWork uow)
        {
            ...
            }))
            {
                ...
                ((Int64Field)insert.IdField)[insert] = null;
                setOwnerID(insert);
