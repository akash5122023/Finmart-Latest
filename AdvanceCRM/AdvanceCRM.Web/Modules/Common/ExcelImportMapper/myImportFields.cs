namespace AdvanceCRM.myImportFields
{
    ...
    public class ExcelImportHelper
    {
        ...
        public static Int32 multiValJoin(IDbConnection myConnection, List<String> myErrors, int row, List<string> expectedHeader, List<dynamic> val)
        {
            ...
            var g = ContactsRow.Fields;
            var currRow = myConnection.TryFirst<ContactsRow>(q => q.Select(g.Id).Where(g.Name == wsField[0] & g.Phone == wsField[1])); //checks if field exists

            if (currRow == null)
            {
                try
                {
                    var newRow = myConnection.InsertAndGetID(new ContactsRow { Name = wsField[0], Phone = wsField[1] }); //create new entry                                
        ...
        public static Int32 oneValJoin(IDbConnection myConnection, List<String> myErrors, int row, string expectedHeader, dynamic val)
        {
            ...
            var g = ContactsRow.Fields;
            var currRow = myConnection.TryFirst<ContactsRow>(q => q.Select(g.Id).Where(g.Name == wsField));

            if (currRow == null)
            {
                try
                {
                    var newRow = myConnection.InsertAndGetID(new ContactsRow { Name = wsField, Phone = "(Imported Field)" }); //create new entry                                
