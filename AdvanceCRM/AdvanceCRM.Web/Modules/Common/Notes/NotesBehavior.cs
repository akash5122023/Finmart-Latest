...
namespace AdvanceCRM.Common
{
    public class NotesBehavior : BaseSaveDeleteBehavior, IImplicitBehavior, IRetrieveBehavior, IFieldBehavior
    {
        ...
        public NotesBehavior(IRequestContext context, ISqlConnections connections)
        {
            _context = context;
        ...
        public void OnReturn(IRetrieveRequestHandler handler)
        {
           ...
           // var notes = new NoteRepository(_context).List(handler.Connection, listRequest);
            var notesResponse = new NoteRepository(_context).List(handler.Connection, listRequest);
            var notes = notesResponse.Entities;
        ...
        private void SaveNote(IUnitOfWork uow, NoteRow note, string entityType, Int64 entityId, Int64? noteId)
        {
            ...
            if (noteId == null)
                new NoteRepository(_context).Create(uow, saveRequest);
            else
                new NoteRepository(_context).Update(uow, saveRequest);
        }

        private void DeleteNote(IUnitOfWork uow, Int64 noteId)
        {
            new NoteRepository(_context).Delete(uow, new DeleteRequest { EntityId = noteId });
        }
        ...
        public override void OnAfterSave(ISaveRequestHandler handler)
        {
            ...
            //var oldList = new NoteRepository(_context).List(handler.Connection, listRequest);
            var oldList = new NoteRepository(_context).List(handler.Connection, listRequest).Entities;
