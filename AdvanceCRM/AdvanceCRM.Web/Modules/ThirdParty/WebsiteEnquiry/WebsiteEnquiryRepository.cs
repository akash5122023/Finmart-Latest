...
namespace AdvanceCRM.ThirdParty.Repositories
{
    ...
    public class WebsiteEnquiryRepository : BaseRepository
    {
        protected ISqlConnections _connections { get; }

        public WebsiteEnquiryRepository(IRequestContext context, ISqlConnections connections)
            : base(context)
        {
            _connections = connections;
        }

        public WebsiteEnquiryRepository(IRequestContext context) : base(context) { }
            ...
            protected override void BeforeSave()
            {
                ...
                using (var connection1 = _connections.NewFor<BizMailConfigRow>())
                {
                   ...
                   .Where(br.UserId == Context.User.GetIdentifier())
                    ...
                    model.EnqRow = connection1.List<BizMailWebRow>(q => q
                          .SelectTableFields()
                          .Select(menq.Rule)
       ...
       private class MyRetrieveHandler : RetrieveRequestHandler<MyRow> { public MyRetrieveHandler(IRequestContext context) : base(context) { } }
        ...
        {
            public List<BizMailWebRow> EnqRow { get; set; }
        }
    }
}