...
namespace AdvanceCRM.ThirdParty.Endpoints
{
    ...
    using MyRepository = Repositories.SIndiaMartDetailsRepository;
    using MyRow = SIndiaMartDetailsRow;
    using System.Linq;

    [Route("Services/ThirdParty/SIndiaMartDetails/[action]")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class SIndiaMartDetailsController : ServiceEndpoint
    {
        private readonly ISqlConnections _connections;
        private readonly IWebHostEnvironment _env;

        public SIndiaMartDetailsController(ISqlConnections connections, IWebHostEnvironment env)
        {
        ...
        public StandardResponse Sync(IUnitOfWork uow)
        {
            ...
            SIndiaMartDetailsRow LastEnquiry;
            ...
            try
            {
                ...
                string uri = "http://mapi.indiamart.com/wservce/enquiry/listing/GLUSR_MOBILE/" + Config.SMobileNumber + "/GLUSR_MOBILE_KEY/" + Config.SapiKey + "/Start_Time/" + StartDate.ToString("dd-MMM-yyyy") + "/End_Time/" + EndDate.ToString("dd-MMM-yyyy") + "/";
                ...
                if (IndiaMartResponsObjects != null)
                {
                    ...
                    foreach (Dictionary<string, object> IndiaMartResponsObject in IndiaMartResponsObjects)
                    {
                        ...
                        Records.Add("IF NOT EXISTS (SELECT * FROM SIndiaMartDetails WHERE QueryId ='" + indiaMartDetail.QUERY_ID + "')" +
                            "INSERT INTO SIndiaMartDetails ([Rn],[QueryId],[QueryType],[SenderName],[SenderEmail],[Subject],[DateRe],[DateR],[DateTimeRe],[GlUserCompanyName],[ReadStatus],[SenderGLUserId],[Mob],[CountryFlag],[QueryModId],[LogTime],[QueryModRefId],[DIRQueryModrefType],[ORGSenderGLUserId],[EnqMessage],[EnqAddress],[EnqCallDuration],[EnqReceiverMob],[EnqCity],[EnqState],[ProductName],[CountryISO],[EmailAlt],[MobileAlt],[Phone],[PhoneAlt],[ImmemberSince],[TotalCnt],[Source]) VALUES " +
                            "('" + indiaMartDetail.RN + "','" +
                    ...
                    if (Records.Count > 0)
                    {
                        using (var innerConnection = _connections.NewFor<SIndiaMartDetailsRow>())
                        {
        ...
        public FileContentResult ListExcel(IDbConnection connection, ListRequest request)
        {
            ...
            var report = new DynamicDataReport(data, request.IncludeColumns, typeof(Columns.SIndiaMartDetailsColumns));
        ...
        public StandardResponse MoveToEnquiry(IUnitOfWork uow, StandardRequest request)
        {
            ...
            var data = new SIndiaMartDetailsRow();

            using (var connection = _connections.NewFor<SIndiaMartDetailsRow>())
            {
                var ind = SIndiaMartDetailsRow.Fields;
                data = connection.TryById<SIndiaMartDetailsRow>(request.Id, q => q
                   .SelectTableFields()
            ...
            try
            {
                using (var connection = _connections.NewFor<ContactsRow>())
                {
                  ...
                  //  GetNextNumberResponse nextNumber = new EnquiryController().GetNextNumber(uow.Connection, new GetNextNumberRequest());
                    ...
                    string str = "INSERT INTO Contacts(ContactType,CustomerType,Name,Phone,Email,Address,AdditionalInfo,OwnerId,AssignedId) VALUES('" + Contacttyp + "','1','" + data.SenderName + "','" + data.Mob + "','" + data.SenderEmail + "','" + data.EnqAddress + "','" + AdditionalCon + "','" + Context.User.GetIdentifier() + "','" + Context.User.GetIdentifier() + "')";
                    ...
                    var sourceMaster = connection.TryFirst<SourceRow>(l => l
                        .Select(s.Id)
                        .Select(s.Source)
                        .Where((s.Source == "IndiaMART2") || (s.Source == "IndiaMart2") || (s.Source == "Indiamart2") || (s.Source == "India MART2") || (s.Source == "India Mart2") || (s.Source == "India mart2"))
                        );

                    if (sourceMaster == null)
                    {
                        response.Status = "Error: IndiaMART2 Source not found in Source master\nKindly add in masters and try again";

                        throw new Exception("IndiaMART2 Source not found in Source master\nKindly add in masters and try again");
                    }
                    ...
                    string Additional = data.GlUserCompanyName + ", Message:" + data.EnqMessage+ ",Feedback:"+ data.Feedback;
                    var str1 = "INSERT INTO Enquiry(ContactsId,Date,Status,AdditionalInfo,SourceId,StageId,OwnerId,AssignedId,EnquiryNo,EnquiryN,CompanyId) VALUES('" + LastContact.Id + "','" + date + "','1','" + Additional + "','" + sourceMaster.Id + "','" + stageMaster.Id + "','" + Context.User.GetIdentifier() + "','" + Context.User.GetIdentifier() + "','" + nextNumber.Serial + "','" + nextNumber.SerialN + "','" + UData.CompanyId + "')";
                    ...
                    connection.Execute("Update SIndiaMartDetails SET IsMoved = 1 WHERE Id = " + request.Id);
                    ...
                    if (IndiaMartSettings.AutoSms.Value == true && !data.Mob.IsNullOrEmpty())
                    {
                        ...
                        //String tempId=IndiaMartSettings.tem
                        msg = msg.Replace("#customername", data.SenderName.IsNullOrEmpty() ? "Customer" : data.SenderName);
                        data.Mob = data.Mob.Replace("-", "").Replace("+91", "").Replace("(", "").Replace(")", "").Replace(" ", "");
                        SMSHelper.SendSMS(data.Mob, msg,msg);
                    }
                }
        ...
        public StandardResponse Sync2(IUnitOfWork uow)
        {
            ...
            try
            {
                ...
                string uri = "https://mapi.indiamart.com/wservce/crm/crmListing/v2/?glusr_crm_key=" + Config.SapiKey + "&start_time=" + StartDate.ToString("dd-MMM-yyyy") + "&end_time=" + EndDate.ToString("dd-MMM-yyyy");
        ...
        public BulkImportResponse BulkMoveToEnquiry(IUnitOfWork uow, BulkRequest request)
        {
            ...
            List<SIndiaMartDetailsRow> data1;
            var ind1 = SIndiaMartDetailsRow.Fields;
            ...
            using (var connection = _connections.NewFor<SIndiaMartDetailsRow>())
            {
                data1 = connection.List<SIndiaMartDetailsRow>(q => q
                   .SelectTableFields()
                ...
                foreach (var item in data1)
                {
                    ...
                    try
                    {
                        ...
                        var sourceMaster = connection.TryFirst<SourceRow>(l => l
                            .Select(s.Id)
                            .Select(s.Source)
                            .Where((s.Source == "IndiaMART1") || (s.Source == "IndiaMart1") || (s.Source == "Indiamart2") || (s.Source == "India MART2") || (s.Source == "India Mart2") || (s.Source == "India mart2"))
                            );
                        ...
                        connection.Execute("Update SIndiaMartDetails SET IsMoved = 1 WHERE Id = " + item.Id);
                        ...
                        if (IndiaMartSettings.AutoSms.Value == true && !item.Mob.IsNullOrEmpty())
                        {
                            ...
                            SMSHelper.SendSMS(item.Mob, msg,tempId);
                        }
