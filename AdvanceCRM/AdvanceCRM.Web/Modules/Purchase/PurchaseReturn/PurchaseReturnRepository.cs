...
namespace AdvanceCRM.Purchase.Repositories
{
    ...
    using System.Data;
    using System.Linq;
    using MyRow = PurchaseReturnRow;
    using Serenity.Extensions.DependencyInjection;

    public class PurchaseReturnRepository : BaseRepository
    {
        protected ISqlConnections _connections { get; }

        public PurchaseReturnRepository(IRequestContext context, ISqlConnections connections)
            : base(context)
        {
            _connections = connections;
        }

        public PurchaseReturnRepository(IRequestContext context)
        ...
        {
            private readonly PurchaseReturnRepository _repository;

            public MySaveHandler(PurchaseReturnRepository repository) : base(repository.Context)
            {
            ...
            protected override void ValidateRequest()
            {
                ...
                if (Row.Products == null || Row.Products.Count == 0)
                    throw new ValidationError("Products", "Please add at least one product detail.");
            }
            ...
            protected override void AfterSave()
            {
                ...
                notify.Module = Masters.NotificationModules.PurchaseReturn;
                notify.InsertDate = System.DateTime.Now;
                notify.InsertUserId = Convert.ToInt32(Context.User.GetIdentifier());
                notify.Url = "/PurchaseReturn#edit/" + Request.Entity.Id;

                notify.Text = Serenity.Authorization.Username + " has" + (this.IsUpdate ? " Updated" : " Created") + " a Purchase Return";
       ...
       private class MyRetrieveHandler : RetrieveRequestHandler<MyRow> { public MyRetrieveHandler(IRequestContext context) : base(context) { } }
        private class MyListHandler : ListRequestHandler<MyRow>
        {
            ...
            public MyListHandler(PurchaseReturnRepository repository) : base(repository.Context)
            {
                _repository = repository;
            }
