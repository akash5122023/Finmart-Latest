...
namespace AdvanceCRM.Services.Endpoints
{
    ...
    using MyRepository = Repositories.TeleCallingAppointmentsRepository;
    using MyRow = TeleCallingAppointmentsRow;

    [Route("Services/Services/TeleCallingAppointments/[action]")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class TeleCallingAppointmentsController : ServiceEndpoint
    {
        private readonly ISqlConnections _connections;
        private readonly IWebHostEnvironment _hostEnvironment;

        public TeleCallingAppointmentsController(ISqlConnections connections, IWebHostEnvironment hostEnvironment)
        {
        ...
        [ServiceAuthorize("Reports:TeleCalling:Appointments")]
        public FileContentResult ListExcel(IDbConnection connection, ListRequest request)
        {
            var data = List(connection, request).Entities;
            var report = new DynamicDataReport(data, request.IncludeColumns, typeof(Columns.TeleCallingAppointmentsColumns));
            var bytes = new ReportRepository().Render(report);
            return ExcelContentResult.Create(bytes, "TeleCallingAppointments_" +
                ...
                if (template.Host != null)
                {
                    ...
                    if (template.Attachment != null)
                    {
                        ...
                        foreach (var f in att)
                        {
                            ...
                            {
                                var filePath = Path.Combine(_hostEnvironment.ContentRootPath, "App_Data", "upload", f["Filename"].ToString());
